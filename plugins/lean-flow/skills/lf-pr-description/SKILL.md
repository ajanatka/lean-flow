...
Why a separate skill: callers need the same writing logic without the single-PR interactive scaffolding that lives in `lf-commit-push-pr`, which runs it inside its full-flow and refresh-mode paths. Extracting keeps one source of truth for the writing principles.

**Naming rationale:** `lf-pr-description`, not `git-pr-description`. Stacking and PR creation are GitHub features; the "PR" in the name refers to the GitHub artifact. Using the `lf-` prefix matches the plugin naming convention for all skills.
...
**Optional `base:<ref>` override (current-branch mode only).** When a caller already knows the intended base branch (e.g., `lf-commit-push-pr` has detected `origin/develop` or `origin/release/2026-04` as the target), it can pass `base:<ref>` to pin the base explicitly. The ref must resolve locally. This overrides auto-detection for current-branch mode; PR mode ignores it (PRs already define their own base via `baseRefName`). Most invocations don't need this — auto-detection (existing PR's `baseRefName` → `origin/HEAD`) covers the common case.

**Examples**:

- `lf-pr-description` → current-branch, no focus, auto-detect base
- `lf-pr-description emphasize the benchmarks` → current-branch, focus = "emphasize the benchmarks"
- `lf-pr-description base:origin/develop` → current-branch, base pinned to `origin/develop`
- `lf-pr-description base:origin/develop emphasize perf` → same + focus
- `lf-pr-description pr:561` → PR #561, no focus
- `lf-pr-description #561 do a good job with the perf story` → PR #561, focus = "do a good job with the perf story"
- `lf-pr-description https://github.com/foo/bar/pull/561 emphasize safety` → PR #561 in foo/bar, focus = "emphasize safety"
...
2. **Current-branch mode or PR mode without an evidence block:** omit the evidence section entirely. If the caller wants to capture evidence, the caller is responsible for gathering it separately and splicing the result in, or for asking this skill to regenerate with updated steering text after capture.
...
5. **Harness badge** -- append a badge footer separated by a `---` rule. Skip if the existing body (for `pr:` input) already contains the badge.
...
```bash
BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/lf-pr-body.XXXXXX") && cat > "$BODY_FILE" <<'__LF_PR_BODY_END__' && echo "$BODY_FILE"
<the composed body markdown goes here, verbatim>
__LF_PR_BODY_END__
```

The quoted sentinel `'__LF_PR_BODY_END__'` keeps `$VAR`, backticks, `${...}`, and any literal `EOF` inside the body from being expanded or clashing with the terminator. Keep `echo "$BODY_FILE"` chained with `&&` so a failed `mktemp` or write never yields a success exit status with a path to a missing file.
