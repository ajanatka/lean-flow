...
[If `Resolve Before Planning` is empty: `-> /lf-plan` for structured implementation planning]
[If `Resolve Before Planning` is not empty: `-> Resume /lf-brainstorm` to resolve blocking questions before planning]
```
...
- What would `lf-plan` still have to invent if this brainstorm ended now?
