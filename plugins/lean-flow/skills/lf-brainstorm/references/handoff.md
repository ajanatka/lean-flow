...
- **4 or fewer visible:** use the platform's blocking question tool (see SKILL.md Interaction Rule 4). This is the default per the plugin AGENTS.md "Interactive Question Tool Design" section.
- **5 or more visible:** render as a numbered list in chat. This is the narrow case-3 overflow exception documented in the same AGENTS.md section; trimming would hide legitimate choices (plan, review, build, refine, pause are all distinct destinations).
...
1. **Plan implementation with `lf-plan` (Recommended)** - Move to `lf-plan` for structured implementation planning. Shown only when `Resolve Before Planning` is empty.
2. **Agent review of requirements doc with `lf-doc-review`** - Dispatch reviewer agents to check the doc for coherence, feasibility, scope, and other persona-specific issues; auto-apply safe fixes; route remaining findings interactively. Shown only when a requirements document exists.
3. **Build it now with `lf-work-lite` (skip planning)** - Skip planning and move to `lf-work-lite`; suited to lightweight, well-defined changes. Shown only when `Resolve Before Planning` is empty **and** scope is lightweight, success criteria are clear, scope boundaries are clear, and no meaningful technical or research questions remain (the "direct-to-work gate").
...
**If user selects "Plan implementation with `lf-plan` (Recommended)":**

Immediately load the `lf-plan` skill in the current session. Pass the requirements document path when one exists; otherwise pass a concise summary of the finalized brainstorm decisions. Do not print the closing summary first.

**If user selects "Agent review of requirements doc with `lf-doc-review`":**

Load the `lf-doc-review` skill, passing the requirements document path as the argument. When lf-doc-review returns "Review complete", return to the Phase 4 options and re-render the menu (the doc may have changed, so re-evaluate `Resolve Before Planning`, direct-to-work gate, and residual findings). If residual P0/P1 findings remain unaddressed, include the post-review nudge above the menu. Do not show the closing summary yet.

**If user selects "Build it now with `lf-work-lite` (skip planning)":**

Immediately load the `lf-work-lite` skill in the current session using the finalized brainstorm output as context. If a compact requirements document exists, pass its path. Do not print the closing summary first.
...
Resume with `lf-brainstorm` when ready to resolve these before planning.
```
