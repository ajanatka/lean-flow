...
- **Write a run artifact** under `.context/lean-flow/lf-code-review/<run-id>/` summarizing findings, applied fixes, residual actionable work, and advisory outputs. Orchestrators read this artifact to route residual `downstream-resolver` findings; the skill itself does not file tickets or prompt the user in autofix.
...
- **Never edit files or externalize work.** Do not write `.context/lean-flow/lf-code-review/<run-id>/`, do not file tickets, and do not commit, push, or create a PR.
...
- **Write a run artifact** under `.context/lean-flow/lf-code-review/<run-id>/` summarizing findings, applied fixes, and advisory outputs. Include the artifact path in the structured output.
...
| `lf-data-migrations-reviewer` | Migrations, schema changes, backfills |
| `lf-data-integrity-guardian` | Migrations, data models, or persistent-data code where safety (constraints, transaction boundaries, privacy) is in play |
| `lf-reliability-reviewer` | Error handling, retries, timeouts, background jobs |
| `lf-adversarial-reviewer` | Diff >=50 changed non-test/non-generated/non-lockfile lines, or auth, payments, data mutations, external APIs |
...
The following paths are lean-flow pipeline artifacts and must never be flagged for deletion, removal, or gitignore by any reviewer:

- `docs/brainstorms/*` -- requirements documents created by lf-brainstorm
- `docs/plans/*.md` -- plan files created by lf-plan (living documents with progress checkboxes)
...
Use the repository portion of the returned PR URL as `<base-repo>` (for example, `your-org/your-repo` from `https://github.com/your-org/your-repo/pull/348`).
...
2. Filter to those whose directory is an ancestor of at least one changed file. A standards file governs all files below it (e.g., `plugins/lean-flow/AGENTS.md` applies to everything under `plugins/lean-flow/`).
...
mkdir -p ".context/lean-flow/lf-code-review/$RUN_ID"
```

Pass `{run_id}` to every persona sub-agent so they can write their full analysis to `.context/lean-flow/lf-code-review/{run_id}/{reviewer_name}.json`.
...
Each persona sub-agent writes full JSON (all schema fields) to `.context/lean-flow/lf-code-review/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
...
**LF always-on agents** (lf-agent-native-reviewer, lf-learnings-researcher) are dispatched as standard Agent calls in parallel with the persona agents. Give them the same review context bundle the personas receive: entry mode, any PR metadata gathered in Stage 1, intent summary, review base branch name when known, `BASE:` marker, file list, diff, and `UNTRACKED:` scope notes. Do not invoke them with a generic "review this" prompt. Their output is unstructured and synthesized separately in Stage 6.
...
11. **Preserve LF agent artifacts.** Keep the learnings, agent-native, schema-drift, and deployment-verification outputs alongside the merged finding set. Do not drop unstructured agent output just because it does not match the persona JSON schema.
...
3. **Spawn validators in parallel.** One sub-agent per finding, dispatched concurrently using the validator template. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at `.context/lean-flow/lf-code-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
...
7. **Learnings & Past Solutions.** Surface lf-learnings-researcher results: if past solutions are relevant, flag them as "Known Pattern" with links to docs/solutions/ files.
8. **Agent-Native Gaps.** Surface lf-agent-native-reviewer results. Omit section if no gaps found.
...
In `mode:headless`, replace the interactive pipe-delimited table report with a structured text envelope: a completion header, a metadata block, findings grouped by `autofix_class`, then trailing sections -- the same structural pattern as document-review's headless output, with lf-code-review's own section headings and per-finding fields.

**Skeleton (fill each line; omit any section with zero items):**

```
Code review complete (headless mode).

Scope / Intent / Reviewers / Verdict / Artifact   <- metadata header; Verdict first for programmatic callers
Applied N safe_auto fixes.

Gated-auto findings (concrete fix, changes behavior/contracts): [needs-verification] on requires_verification:true
Manual findings (actionable, needs handoff)
Advisory findings (report-only)                    <- owner:release also lands here
Pre-existing issues                                <- regardless of autofix_class
Residual risks / Learnings & Past Solutions / Agent-Native Gaps / Testing gaps
Coverage: suppressed / demotion-suppressed / validator drops (+reasons) / over-budget drops / untracked-excluded / failed reviewers

Review complete
```

Each finding line: `[P#][autofix_class -> owner][needs-verification] File: <file:line> -- <title> (<reviewer>, confidence <N>)` followed by indented `Why:` / `Suggested fix:` / `Evidence:` lines.

**Field tiers and enrichment:** `Why:` and `Evidence:` are detail-tier -- after merge (Stage 5), load them from the per-agent artifact files under `.context/lean-flow/lf-code-review/{run_id}/` for findings that survived dedup and confidence gating, matching on `file + line_bucket(line, +/-3)` (same tolerance as Stage 5 dedup) with `normalize(title)` as a tie-breaker, trying contributing reviewers in list order. `Suggested fix:` is merge-tier -- populate directly from the compact return, no artifact lookup needed. No-match (artifact write failed, or finding synthesized during merge): omit `Why:`/`Evidence:` and note the gap in Coverage; `Suggested fix:` still populates.

**Rules:** the `Artifact:` line points to the full run artifact (debugging/full-fidelity access; the text envelope is the primary handoff). If all reviewers fail or time out, emit `Code review degraded (headless mode). Reason: 0 of N reviewers returned results.` before "Review complete". Always end with "Review complete" as the terminal signal.
...
7. **Live-boundary evidence:** if the diff adds raw SQL (`text()`/`execute`), storage or network I/O, or new Tailwind classes — require a `requires_db` test, browser walk, or `make css` verification respectively; mocked tests alone are insufficient for these.
...
- In interactive, autofix, and headless modes, write a per-run artifact under `.context/lean-flow/lf-code-review/<run-id>/` containing:
