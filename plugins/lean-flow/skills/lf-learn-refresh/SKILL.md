...
Follow the same interaction style as `lf-brainstorm`:
...
Run `lf-learn` after solving problems to start building your knowledge base.
```
...
- **Update territory** — file paths moved, classes renamed, links broke, metadata drifted, but the core recommended approach is still how the code works. `lf-learn-refresh` fixes these directly.
- **Replace territory** — the recommended solution conflicts with current code, the architectural approach changed, or the pattern is no longer the preferred way. This means a new learning needs to be written. A replacement subagent writes the successor following `lf-learn`'s document format (frontmatter, problem, root cause, solution, prevention), using the investigation evidence already gathered. The orchestrator does not rewrite learnings inline — it delegates to a subagent for context isolation.
...
- **Insufficient evidence** — the drift is so fundamental that you cannot confidently document the current approach. The entire subsystem was replaced, or the new architecture is too complex to understand from a file scan alone. → Mark as stale in place:
   ...
   - Recommend the user run `lf-learn` after their next encounter with that area, when they have fresh problem-solving context
...
- `../lf-learn/references/schema.yaml` — frontmatter fields and enum values (canonical copy lives in the lf-learn skill)
- Read the shared schema at `../lf-learn/references/yaml-schema.md` (canonical copy lives in the lf-learn skill) — category mapping
- Read the shared template at `../lf-learn/assets/resolution-template.md` (canonical copy lives in the lf-learn skill) — section structure
...
2. The subagent writes the new learning using the support files as the source of truth: the shared `../lf-learn/references/schema.yaml` (canonical copy in lf-learn) for frontmatter fields and enum values, the shared `../lf-learn/references/yaml-schema.md` (canonical copy lives in the lf-learn skill) for category mapping and YAML-safety rules for array items, and the shared `../lf-learn/assets/resolution-template.md` (canonical copy lives in the lf-learn skill) for section order. It should use dedicated file search and read tools if it needs additional context beyond what was passed.
...
3. Recommend the user run `lf-learn` after their next encounter with that area
...
If any files were created, updated, consolidated, replaced, or deleted in this pass, regenerate the lookup index after all other Phase 4 actions complete: `python3 ../lf-learn/scripts/build_solutions_index.py docs/solutions` (INDEX.md is the cheap first-pass lookup surface — it must never go stale). Skip this step only when every doc was classified Keep (no files modified).
...
2. Whether the working tree has other uncommitted changes beyond what learning-refresh modified
...
Stage only the files that learning-refresh modified — not other dirty files in the working tree.

### Interactive mode

First, run `git branch --show-current` to determine the current branch. Then present the correct options based on the result. Stage only learning-refresh files regardless of which option the user picks.

**If the current branch is main, master, or the repo's default branch:**

1. Create a branch, commit, and open a PR (recommended) — the branch name should be specific to what was refreshed, not generic (e.g., `docs/refresh-auth-learnings` not `docs/learning-refresh`)
...
1. Commit only the learning-refresh changes to `{current branch name}` (selective staging — other dirty files stay untouched)
2. Don't commit
...
## Relationship to lf-learn

- `lf-learn` captures a newly solved, verified problem
- `lf-learn-refresh` maintains older learnings as the codebase evolves — both their individual accuracy and their collective design as a document set

Use **Replace** only when the refresh process has enough real evidence to write a trustworthy successor. When evidence is insufficient, mark as stale and recommend `lf-learn` for when the user next encounters that problem area.

Use **Consolidate** proactively when the document set has grown organically and redundancy has crept in. Every `lf-learn` invocation adds a new doc — over time, multiple docs may cover the same problem from slightly different angles. Periodic consolidation keeps the document set lean and authoritative.

## Discoverability Check

After the refresh report is generated, check whether the project's instruction files (AGENTS.md/CLAUDE.md) would lead an agent to discover and search `docs/solutions/` before starting work in a documented area, and if not, draft the smallest natural addition that fixes that. This runs every time. See `lean-flow:lf-learn`'s "Discoverability Check" section for the full assessment criteria, calibration examples, and consent flow — the mechanics are identical here, with two refresh-specific differences: the mode names are **interactive** (ask via the blocking question tool) and **autofix** (report only, no instruction-file edit — this replaces `lf-learn`'s full/lightweight mode split), and any resulting edit is committed per step 5 below rather than inline.

**Amend or create a follow-up commit when the check produces edits.** If the discoverability check resulted in an edit to an instruction file and Phase 5 already committed the refresh changes, stage the newly edited file and either amend the existing commit (if still on the same branch and no push has occurred) or create a small follow-up commit (e.g., `docs: add docs/solutions/ discoverability to AGENTS.md`). If Phase 5 already pushed the branch to a remote (e.g., the branch+PR path), push the follow-up commit as well so the open PR includes the discoverability change. This keeps the working tree clean and the remote in sync at the end of the run. If the user chose "Don't commit" in Phase 5, leave the instruction-file edit unstaged alongside the other uncommitted refresh changes — no separate commit logic needed.
