...
**Headless mode** changes the interaction model, not the classification boundaries. lf-doc-review still applies the same judgment about which tier each finding belongs in. The only difference is how non-safe_auto findings are delivered:
...
```
Skill("lf-doc-review", "mode:headless docs/plans/my-plan.md")
```
...
**If no document is specified (headless mode):** Output "Review failed: headless mode requires a document path. Re-invoke with: Skill(\"lf-doc-review\", \"mode:headless <path>\")" without dispatching agents.
...
```
Reviewing with:
- lf-coherence-reviewer (always-on)
- lf-feasibility-reviewer (always-on)
- lf-scope-guardian-reviewer -- plan has 12 requirements across 3 priority levels
- lf-security-lens-reviewer -- plan adds API endpoints with auth flow
```

### Build Agent List

Always include:
- `lf-coherence-reviewer`
- `lf-feasibility-reviewer`

Add activated conditional personas:
- `lf-product-lens-reviewer`
- `lf-design-lens-reviewer`
- `lf-security-lens-reviewer`
- `lf-scope-guardian-reviewer`
- `lf-adversarial-document-reviewer`
...
### Code-surface verification (required)

Every reviewer that evaluates feasibility or correctness (at minimum `lf-feasibility-reviewer` and, when dispatched, `lf-adversarial-document-reviewer`) MUST grep or open the actual code surfaces the document cites -- files, enums, schemas, API shapes -- and verify they exist in the stated shape. Reviewing the prose alone is not a review. Findings of "cited code doesn't match" are automatic P1. Include this instruction in the dispatch prompt for these personas, not just as an orchestrator-side check.
...
Cross-session persistence is out of scope. A new invocation of lf-doc-review on the same document starts with a fresh round 1 and no carried primer, even if prior sessions deferred findings into the document's Open Questions section.
