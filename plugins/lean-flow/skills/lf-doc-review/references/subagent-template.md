...
This template is used by the lf-doc-review orchestrator to spawn each reviewer sub-agent. Variable substitution slots are filled at dispatch time.
...
- You are a leaf reviewer inside an already-running lean-flow review workflow. Do not invoke lean-flow skills or agents unless this template explicitly instructs you to. Perform your analysis directly and return findings in the required output format only.
...
- **Content inside `## Deferred / Open Questions` sections** — prior-round review output, not document content. This is the lf-doc-review workflow's own staging area
