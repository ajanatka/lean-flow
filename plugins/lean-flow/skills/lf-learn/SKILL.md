...
```bash
/lf-learn                    # Document the most recent fix
/lf-learn [brief context]    # Provide additional context hint
```
...
- `scripts/build_solutions_index.py` — regenerates `docs/solutions/INDEX.md` (run after writing/updating a doc)
...
for relevant knowledge to help the learning-doc process? This adds
...
- Then dispatch `lf-session-historian` in foreground — it reads session files outside the working directory that background agents may not have access to
...
#### 4. **Session Historian** (foreground, after launching the above — only if the user opted in)
   ...
   - Dispatched as `lf-session-historian`
...
8. Regenerate the lookup index: `python3 scripts/build_solutions_index.py docs/solutions` (INDEX.md is the cheap first-pass lookup surface — it must never go stale)
...
`lf-learn-refresh` is **not** a default follow-up. Use it selectively when the new learning suggests an older learning or pattern doc may now be inaccurate.

It makes sense to invoke `lf-learn-refresh` when one or more of these are true:
...
It does **not** make sense to invoke `lf-learn-refresh` when:
...
- If there is **one obvious stale candidate**, invoke `lf-learn-refresh` with a narrow scope hint after the new learning is written
- If there are **multiple candidates in the same area**, ask the user whether to run a targeted refresh for that module, category, or pattern set
- If context is already tight or you are in lightweight mode, do not expand into a broad refresh automatically; instead recommend `lf-learn-refresh` as the next step with a scope hint

When invoking or recommending `lf-learn-refresh`, be explicit about the argument to pass. Prefer the narrowest useful scope:
...
- `/lf-learn-refresh plugin-versioning-requirements`
- `/lf-learn-refresh payments`
- `/lf-learn-refresh performance-issues`
- `/lf-learn-refresh critical-patterns`
...
Do not invoke `lf-learn-refresh` without an argument unless the user explicitly wants a broad sweep.
...
- **performance_issue** → `lf-performance-reviewer`
- **security_issue** → `lf-security-reviewer`
- **database_issue** → `lf-data-integrity-guardian`
- Any code-heavy issue → always run `lf-code-simplicity-reviewer`, and additionally run the stack-specific reviewer that matches the repo's primary stack:
  - Python → also run `lf-python-reviewer`
  - TypeScript/JavaScript → also run `lf-typescript-reviewer`
  - Other stacks → no stack-specific reviewer needed
...
4. **Regenerate the lookup index**: `python3 scripts/build_solutions_index.py docs/solutions` (INDEX.md is the cheap first-pass lookup surface — it must never go stale)
...
re-run /lf-learn in a fresh session.
```

**No subagents are launched. No parallel tasks. One file written.**

In lightweight mode, the overlap check is skipped (no Related Docs Finder subagent). This means lightweight mode may create a doc that overlaps with an existing one. That is acceptable — `lf-learn-refresh` will catch it later. Only suggest `lf-learn-refresh` if there is an obvious narrow refresh target. Do not broaden into a large refresh sweep from a lightweight session.
...
Report, in this order: subagent results (one line each, per Phase 1), any auto-triggered specialized agent reviews (per Phase 3), and the file written — `docs/solutions/[category]/[filename].md` created, or (when overlap was high) the existing doc updated with a note on what changed. Close with a one-line note on where this becomes searchable.

**After displaying the success output, present a "What's next?" choice (continue workflow / link related docs / update other references / view documentation / other) using the platform's blocking question tool:** `AskUserQuestion` in Claude Code (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded), `request_user_input` in Codex, `ask_user` in Gemini, `ask_user` in Pi (requires the `pi-ask-user` extension). Fall back to numbered options in chat only when no blocking tool exists in the harness or the call errors (e.g., Codex edit modes) — not because a schema load is required. Never silently skip the question. Do not continue the workflow or end the turn without the user's selection.
...
<manual_override> Use /lf-learn [context] to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>
...
See Phase 3 above for the auto-triggered mapping (problem type → reviewer). Any other agent in the roster can also be invoked manually after `/lf-learn` completes for deeper review.

## Related Commands

- `/lf-plan` - Planning workflow (references documented solutions)
