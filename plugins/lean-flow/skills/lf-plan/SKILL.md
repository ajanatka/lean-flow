---
name: lf-plan
description: "Create structured plans for any multi-step task -- software features, research workflows, events, study plans, or any goal that benefits from structured breakdown. Also deepen existing plans with interactive review of sub-agent findings. Use for plan creation when the user says 'plan this', 'create a plan', 'write a tech plan', 'plan the implementation', 'how should we build', 'what's the approach for', 'break this down', 'plan a trip', 'create a study plan', or when a brainstorm/requirements document is ready for planning. Use for plan deepening when the user says 'deepen the plan', 'deepen my plan', 'deepening pass', or uses 'deepen' in reference to a plan. For exploratory or ambiguous requests where the user is unsure what to do, prefer lf-brainstorm first."
...
`lf-brainstorm` defines **WHAT** to build. `lf-plan` defines **HOW** to build it. `lf-work-lite` executes the plan. A prior brainstorm is useful context but never required — `lf-plan` works from any input: a requirements doc, a bug report, a feature idea, or a rough description.
...
This workflow produces a durable implementation plan. It does **not** implement code, run tests, or learn from execution-time results. If the answer depends on changing code and seeing what happens, that belongs in `lf-work-lite`, not here.
...
1. **Use requirements as the source of truth** - If `lf-brainstorm` produced a requirements document, planning should build from it rather than re-inventing behavior.
...
- If the ambiguity is mainly product framing, user behavior, or scope definition, recommend `lf-brainstorm` as a suggestion — but always offer to continue planning here as well
...
- **Symptom without a root cause** (user describes broken behavior but hasn't identified why) — announce that investigation is needed before planning and load the `systematic-debugging` skill. A plan requires a known problem to solve; debugging identifies what that problem is. Announce the routing clearly: "This needs investigation before planning — switching to systematic-debugging to find the root cause."
- **Clear task ready to execute** (known root cause, obvious fix, no architectural decisions) — suggest `lf-work-lite` as a faster alternative alongside continuing with planning. The user decides.
...
- Ask the user, using the platform's blocking question tool when available (see Interaction Method), whether to:
  1. Resume `lf-brainstorm` to resolve them
...
- Task lf-repo-research-analyst(Scope: technology, architecture, patterns. {planning context summary})
- Task lf-learnings-researcher(planning context summary)
Collect:
...
**Leverage lf-repo-research-analyst's technology context:**

The lf-repo-research-analyst output includes a structured Technology & Infrastructure summary. Use it to make sharper external research decisions:

- If specific frameworks and versions were detected (e.g., Rails 7.2, Next.js 14, Go 1.22), pass those exact identifiers to lf-best-practices-researcher so it fetches version-specific documentation
...
If Step 1.2 indicates external research is useful, run:

- Task lf-best-practices-researcher(planning context summary)
...
- Task lf-spec-flow-analyzer(planning context summary, research findings)
...
Each unit carries a stable plan-local **U-ID** assigned in Phase 3.5 (`U1`, `U2`, …). U-IDs survive reordering, splitting, and deletion: new units take the next unused number, gaps are fine, and existing IDs are never renumbered. This lets `lf-work-lite` reference units unambiguously across plan edits.
...
<!-- Each unit carries a stable plan-local U-ID (U1, U2, …) assigned sequentially.
     ...
     a gap. This anchor is what lf-work-lite references in blockers and verification, so
     stability across plan edits is load-bearing. -->

- [ ] U1. **[Name]**

<!-- Fill in the full field set defined in Phase 3.5 (Goal, Requirements, Dependencies,
     Files, Approach, Execution note, Technical design, Patterns to follow, Test scenarios,
     Verification) -- not repeated here to avoid duplicating that section. -->
...
- **Horizontal rules (`---`) between top-level sections** in Standard and Deep plans, mirroring the `lf-brainstorm` requirements doc convention. Improves scannability of dense plans where many H2 sections sit close together. Omit for Lightweight plans where the whole doc fits on a single screen.
...
**REQUIRED — verify every cited code reference against the live codebase.** For every file:line, function, enum, schema, or API shape the plan cites: grep/read the live code and confirm it exists in the stated shape TODAY. A plan that cites code nobody opened is unreviewed. This step is mandatory, not a deepening option — it runs regardless of plan depth or confidence-check outcome, and it runs before the checklist below.

Before finalizing, check:
- The plan does not invent product behavior that should have been defined in `lf-brainstorm`
...
- Blocking questions were either resolved, explicitly assumed, or sent back to `lf-brainstorm`
...
`lf-doc-review` and this confidence check are different:
- Use the `lf-doc-review` skill when the document needs clarity, simplification, completeness, or scope control
...
**Load `references/plan-handoff.md` now.** It contains the full instructions for 5.3.8 (document review), 5.3.9 (final checks and cleanup), and 5.4 (post-generation handoff, including Issue Creation branching). Document review is mandatory — do not skip it even if the confidence check already ran.
...
**Options:**
1. **Start `/lf-work-lite`** (recommended) - Begin implementing this plan in the current session
...
Routing each selection and contextual surfacing of residual document-review findings live in `references/plan-handoff.md` — follow it for every branch.
