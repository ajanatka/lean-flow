...
This file is loaded when lf-plan detects a non-software task (Phase 0.1b). It replaces the software-specific phases (0.2 through 5.1) with a domain-agnostic planning workflow.
...
Once past these checks, commit to producing a plan. Do not exit because the task looks like a "lookup" or "research question" — the user invoked `lf-plan` because they want a structured output.
...
When research is recommended, do it — don't just offer. Stale recommendations (closed restaurants, rethemed attractions, outdated prices) are worse than no recommendations. The user invoked `/lf-plan` because they want a good plan, not a disclaimer about training data.
...
2. **Print in chat** — Render the plan directly in the conversation for the user to copy or act on.

Do not offer `/lf-work-lite` (software-only) or issue creation (not applicable to non-software plans).
