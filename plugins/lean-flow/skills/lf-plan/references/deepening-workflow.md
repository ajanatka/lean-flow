...
- `lf-spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `lf-repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `lf-learnings-researcher` for institutional knowledge and past solved problems
- `lf-best-practices-researcher` for current external patterns, official framework/library behavior, and industry guidance

**Key Technical Decisions**
- `lf-architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `lf-best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `lf-architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `lf-repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `lf-best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `lf-repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues, including consistency and duplication risks against existing patterns
- Add `lf-spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `lf-architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `lf-performance-reviewer` for scalability, latency, throughput, and resource-risk analysis
  - `lf-security-reviewer` for auth, validation, exploit surfaces, and security boundary review
  - `lf-data-integrity-guardian` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `lf-security-reviewer` for security, auth, privacy, and exploit risk
  - `lf-data-integrity-guardian` for persistent data safety, constraints, and transaction boundaries
  - `lf-data-migrations-reviewer` for migration realism, backfills, and production data transformation risk
  - `lf-deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `lf-performance-reviewer` for capacity, latency, and scaling concerns
...
```bash
SCRATCH_DIR="$(mktemp -d -t lf-plan-deepen-XXXXXX)"
echo "$SCRATCH_DIR"
```
...
1. **Summarize the agent and its target section** — e.g., "The lf-architecture-strategist reviewed Key Technical Decisions and found:"
...
- Reorder or split implementation units when sequencing is weak — but **never renumber existing U-IDs**. Reordering preserves U-IDs in their new order (e.g., U1, U3, U5 reordered is correct; renumbering to U1, U2, U3 is not). Splitting keeps the original U-ID on the original concept and assigns the next unused number to the new unit. Renumbering breaks lf-work-lite blocker and verification references that were written against the original IDs
...
- Recommend `lf-brainstorm` if the gap is truly product-defining
