...
2. For each changed file, check its ancestor directories up to the repo root for standards files. A file like `plugins/lean-flow/AGENTS.md` applies to all changes under `plugins/lean-flow/`.
...
- **Broken cross-references** -- agent names that are not fully qualified (e.g., `lf-learnings-researcher` instead of `lf-learnings-researcher`). Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
