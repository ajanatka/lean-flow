...
This agent is not currently wired into a default skill flow.

Skills that need structured external grounding (for example, `lf-brainstorm` or `lf-plan` external research stages) can adopt this agent in follow-up work; the output contract above is stable.
