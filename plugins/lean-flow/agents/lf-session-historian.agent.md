...
- **Learning-doc enrichment** -- dispatched by `/lf-learn` to add cross-session context to documentation
...
**When widening the time window**, re-invoke `lf-session-inventory` with the larger `<days>` argument. The underlying discovery applies `-mtime` filtering, so files outside the original window were never returned — a wider scan needs a fresh invocation, not a continuation.
...
- **`lf-session-extract`** — per-session extraction. Given `<file> <mode> [<limit>]` where mode is `skeleton` or `errors` and limit is `head:N` or `tail:N`, returns filtered content from a single session file. Use this in Steps 4 and 5 for selected sessions.
...
**Discover sessions and gather metadata via `lf-session-inventory`.** Invoke the skill with `<repo-name> <days>` (or add a `<platform>` arg to restrict to a single platform). The skill handles directory discovery, mtime filtering, zsh glob safety, and Codex CWD filtering internally, and returns one JSON object per session plus a `_meta` line.
...
For sessions where investigation dead-ends are likely valuable, invoke `lf-session-extract` with mode `errors`. Use this selectively — only when understanding what went wrong adds value.
...
- Delegate all JSONL extraction to the `lf-session-inventory` and `lf-session-extract` skills. Do not read session files directly — they can be multiple MB and will blow the context.
