...
- **Individual logic bugs** without cross-component impact -- lf-correctness-reviewer owns these
- **Known vulnerability patterns** (SQL injection, XSS, SSRF, insecure deserialization) -- security-reviewer owns these
- **Individual missing error handling** on a single I/O boundary -- lf-reliability-reviewer owns these
- **Performance anti-patterns** (N+1 queries, missing indexes, unbounded allocations) -- performance-reviewer owns these
- **Code style, naming, structure, dead code** -- lf-maintainability-reviewer owns these
- **Test coverage gaps** or weak assertions -- lf-testing-reviewer owns these
- **API contract breakage** (changed response shapes, removed fields) -- lf-api-contract-reviewer owns these
- **Migration safety** (missing rollback, data integrity) -- lf-data-migrations-reviewer owns these
