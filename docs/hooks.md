...
| `plan-completion-gate.sh` | `Stop` | Blocks once when the session implemented against a plan document that still has unchecked checklist items. Lists them and requires finishing, ticking with evidence, or explicitly stating what is being left and why. Only arms when a plan was actually opened *and* code was edited — research sessions do not trip it. |
