...
- **A balanced-tier model (Claude Code: Sonnet; Codex: GPT-5.6 Terra) implements from bounded briefs.** Once the orchestrator has decomposed a task into a self-contained handoff packet, a cheaper model executes it. Claude's compatibility alias is `sonnet-worker`; Codex uses `lf-implementation-worker`.
- **A fast-tier model (Claude Code: Haiku; Codex: GPT-5.6 Luna) scans, reads, and reduces.** File discovery, grep sweeps, log reduction, config reads, inventory tasks — anything where the orchestrator needs a conclusion, not a raw dump. Claude's alias is `scan-worker`; Codex uses `lf-scan-worker`.
...
- **Hard-trigger reviewer-model escalation is a deliberate, bounded exception to "escalate on evidence, not prestige."** `lf-code-review`'s persona reviewers run on the balanced tier by default (Claude Code: Sonnet; Codex: GPT-5.6 Terra), but hard-trigger correctness plus security/adversarial review runs on the frontier tier (Claude Code: Opus; Codex: GPT-5.6 Sol), per-call, regardless of whether a cheaper attempt failed first. This is pre-emptive because model depth changes findings on hard-to-unwind diffs. The exception is narrow: exactly the two selected hard-trigger personas, only on hard triggers, and it does not change how the orchestrator itself is routed.
